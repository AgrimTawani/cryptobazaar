...
export default function OrderCreatedEmail({ sellerName, amount, asset, pricePerUnit, orderId, displayId }: Props) {
  ...
  return (
    <BaseEmail
      previewText={`Your listing for ${amount} ${asset} is live on CryptoBazaar`}
    ...
    >
      ...
      <Text style={{ color: "#444", fontSize: 15, lineHeight: 1.6 }}>
        Hi {sellerName}, your listing to sell <strong>{amount} {asset}</strong> at ₹{pricePerUnit}/unit is now live on the CryptoBazaar marketplace. We will notify you instantly when a buyer locks your order.
      </Text>
