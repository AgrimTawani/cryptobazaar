...
export default function DisputeRaisedEmail({ role, name, amount, asset, orderId, displayId, raisedByRole }: Props) {
    ...
    return (
      <BaseEmail previewText={`Dispute raised on ${amount} ${asset} order`} ctaLabel="Review Dispute" ctaUrl={url}>
        ...
        <Text style={{ color: "#444", fontSize: 15, lineHeight: 1.6 }}>
          A dispute has been raised by the <strong>{raisedByRole}</strong> on a <strong>{amount} {asset}</strong> order (#{displayId}). Please review and resolve within 24 hours.
        </Text>
      </BaseEmail>
    );
  }
  return (
    <BaseEmail previewText="A dispute has been raised on your trade" ctaLabel="View Dispute" ctaUrl={url}>
      ...
      <Text style={{ color: "#444", fontSize: 15, lineHeight: 1.6 }}>
        Hi {name}, a dispute has been raised on your <strong>{amount} {asset}</strong> trade (Order #{displayId}). Our admin team will review all evidence and resolve within 24 hours. Please do not make any further payments.
      </Text>
    </BaseEmail>
  );
}
