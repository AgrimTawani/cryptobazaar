...
def verify_identity(raw_text: str, extracted_metadata: dict,
    ...
    if user_name:
        ...
        if extracted_name:
            ...
            # Try to find name anywhere in raw text
            if user_name.lower() in raw_text.lower():
                result["verification"]["name_match"] = "MATCHED"
                result["extractedName"] = user_name
            else:
                # Fuzzy search across text blocks
                best_ratio = 0
                best_match = None
...
def check_a1_account_age(df: pd.DataFrame) -> dict:
    """A1: Account Age — date span of statement."""
    if df.empty:
...
def check_a2_regular_income(df: pd.DataFrame) -> dict:
    """A2: Regular Income Credit — recurring salary/inflows per month."""
    if df.empty:
...
def check_a4_transaction_diversity(df: pd.DataFrame) -> dict:
    """A4: Transaction Type Diversity — different payment modes."""
    if df.empty:
...
def _get_crypto_txns(df: pd.DataFrame) -> pd.DataFrame:
    """Filter transactions matching crypto exchange keywords."""
    if df.empty:
        return df
...
def check_b2_trading_frequency(df: pd.DataFrame) -> dict:
    """B2: Trading Frequency — unique months with crypto activity."""
    crypto = _get_crypto_txns(df)
    if crypto.empty:
...
def check_b3_bidirectional_flow(df: pd.DataFrame) -> dict:
    """B3: Bidirectional Flow — both buy and sell crypto activity."""
    crypto = _get_crypto_txns(df)
    if crypto.empty:
...
def check_b4_trade_volume_consistency(df: pd.DataFrame) -> dict:
    """B4: Trade Volume Consistency — no monthly spike > 5x average."""
    crypto = _get_crypto_txns(df)
    if crypto.empty:
...
def _extract_sender(narration: str) -> Optional[str]:
    """Extract sender identity from narration text."""
    narration = str(narration).strip()
    upper = narration.upper()

    # UPI pattern: UPI/CR/REF/SENDER_NAME/... or UPI-SENDER-...
    upi_match = re.search(r'UPI[/\-](?:CR|DR)?[/\-]?[A-Z0-9]+[/\-](.+?)(?:[/\-]|$)', upper)
    if upi_match:
        return upi_match.group(1).strip()

    # NEFT pattern: NEFT/SENDER_NAME or NEFT-IFSC-SENDER
    ...
    # Fallback: use narration itself (trimmed)
    cleaned = re.sub(r'\d{6,}', '', narration)  # remove long numbers
    cleaned = re.sub(r'\s+', ' ', cleaned).strip()
    return cleaned[:50] if cleaned else None


def check_c1_unique_sender_count(df: pd.DataFrame) -> dict:
    """C1: Unique Inflow Sender Count — avg unique new senders per month."""
    credits = df[df['Deposit'] > 0].copy()
    if credits.empty:
...
def check_c2_sender_recurrence(df: pd.DataFrame) -> dict:
    """C2: Sender Recurrence Rate — % of senders appearing in >1 month."""
    credits = df[df['Deposit'] > 0].copy()
    if credits.empty:
    ...
    # For each sender, count unique months they appear in
...
def check_c3_credit_debit_velocity(df: pd.DataFrame) -> dict:
    """C3: Credit-to-Debit Velocity — avg time between credit and next debit."""
...
def check_c4_round_number_concentration(df: pd.DataFrame) -> dict:
    """C4: Round Number Concentration — % of inflows that are exact round numbers."""
    credits = df[df['Deposit'] > 0].copy()
    if credits.empty:
    ...
    total = len(credits)
    # Round numbers: exact multiples of 1000 (e.g., 5000, 10000, 50000, 100000)
...
def check_c5_structuring_pattern(df: pd.DataFrame) -> dict:
    """C5: Structuring Pattern — same-day multi-sender credits < 50K summing high."""
    credits = df[df['Deposit'] > 0].copy()
    if credits.empty:
...
def check_c6_inflow_spike(df: pd.DataFrame) -> dict:
    """C6: Inflow Spike Detection — monthly inflow vs 3x average."""
    credits = df[df['Deposit'] > 0].copy()
    if credits.empty:
...
def check_d2_balance_consistency(df: pd.DataFrame) -> dict:
    """D2: Balance Consistency — balance drops below 500 INR."""
    if df.empty:
...
def check_d4_net_flow_sign(df: pd.DataFrame) -> dict:
    """D4: Net Flow Sign — positive net flow months."""
    if df.empty:
